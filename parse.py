...
def get_column_key_to_datetime_dict(tab_data: pd.DataFrame):
    column_key_to_datetime_dict: dict[Hashable, datetime] = {}
...
def parse_target_name_row(tab_data: pd.DataFrame,
                          date_time_dict: dict[Hashable, datetime]):
        ...
        if name == TARTET_NAME:
            for key in date_time_dict.keys():
                ...
                if type(item) != str:
                    continue
                if 'HO' in item:
                    add_ho_event(event_name=item,
                                 cur_datetime=date_time_dict[key])
